<template>
    <div class="model-upload-img-warp">
        <input type="file" style="display: none" ref="input" @change="inputChange($event)" :accept="acceptTypeList"
        ...
        <div class="clearfix">
            ...
            <div class="uploaded-image pointer" v-for="(item, index) in fileList"
                ...
                <div class="hover-container" v-show="item.progress>=100">
                    ...
                    <p class="set-p" @click="setCover(index)" v-show="!item.isCover && needCover">Set as Cover</p>
                </div>
                <div class="picture-cover">
                    <img src="../../assets/images/icon-cover.png" alt="" class="img-cover" v-show="item.isCover && needCover">
                </div>
...
<script>
    ...
    export default {
        name: 'editor-model-concepts-upload',
        mixins: [fileControl],
        props: {
            needCover: {
                type: Boolean
        ...
        created() {
            if(this.propImageList.length){
                this.propImageList.forEach((img, index)=>{
                    let _img = new File({
                        id: randomString(),
                        name: img.dir,
                        isCorrectFile: true,
                    ...
                    _img.setCustomValue({src: img.src, isCover: img.iscover, width: img.width, height: img.height});
                    this.fileList.push(_img);
                    if (img.isCover) {
                        this.setInitCover(index);
                    }
                });
